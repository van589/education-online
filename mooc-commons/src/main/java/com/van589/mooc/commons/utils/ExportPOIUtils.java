...
public class ExportPOIUtils {
     ...
     * @return
     */
    public static Workbook start_download(List<?> projects, String[] columnNames, String[] keys){
        //将集合中对象的属性  对应到  List<Map<String,Object>>
        List<Map<String, Object>> list = createExcelRecord(projects, keys);
        //将转换成的Workbook对象通过流形式下载
        Workbook workbook = createWorkBook(list, keys, columnNames);
        return workbook;
     ...
     * @param fileName
     */
    public static List<Map<String,Object>> readExcelFile(List<Map<String,Object>> list ,InputStream inputStream, String fileName) throws IOException {
            ...
            for (int i = 0; i < numOfSheet; i++) {
                ...
                for (int j = 1; j <= lastRowNum; j++) {
                    ...
                    for (int k = 0; k <= lastCellNum; k++) {
                        ...
                        String res = row.getCell(k).getStringCellValue().trim(); //获取当前行某列的值
                        String columnValues = columnNames.getCell(k).getStringCellValue().trim();//获取当前行的列名
                        map.put(columnValues,res);
                    }
                    list.add(map);
                }
            }
